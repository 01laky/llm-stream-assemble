...
describe("openaiCompatibleAdapter cloudflare manifest golden fixtures", () => {
	const manifest = loadHostFixtureManifest("cloudflare");

	for (const [name, entry] of Object.entries(manifest)) {
		if (!entry.goldenTestId) continue;

		it(`LSA-${entry.goldenTestId}: cloudflare/${name} matches expected events`, async () => {
			const adapterOptions = entry.adapterOptions ?? {};
			if (entry.kind === "response") {
				expect(responseHostFixture("cloudflare", name)).toEqual(
					expectedHostCompatibleEvents("cloudflare", name),
				);
				return;
			}
			await expect(
				streamHostFixtureWithAdapter("cloudflare", name, adapterOptions),
			).resolves.toEqual(expectedHostCompatibleEvents("cloudflare", name));
		});
	}
});
