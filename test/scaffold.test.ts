...
describe("LSA-S06: assembleStream empty input", () => {
	it("yields finish incomplete for empty streams", async () => {
		const adapter: StreamAdapter = { parseChunk: () => [] };
		const events = [];
		for await (const event of assembleStream(emptyAsyncIterable<string>(), adapter)) {
			events.push(event);
		}
		expect(events).toEqual([{ type: "finish", reason: "incomplete" }]);
	});
});
