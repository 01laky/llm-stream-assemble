...
¹⁵ OpenAI Chat / compatible logprobs require `logprobs: true` on the request → typed **`logprob`** events (**LSA-LP01**–**LP75**, **OC296**–**OC318**).
¹⁶ OpenAI Responses logprobs require `include: ["message.output_text.logprobs"]` (optional `top_logprobs`) → typed **`logprob`** events (**LSA-RL01**–**RL52**, **R71**–**R99**).
...
| OpenAI Responses      | Logprobs omitted unless requested                                   | Set `include: ["message.output_text.logprobs"]` (+ optional `top_logprobs`) → typed **`logprob`** events (**LSA-RL01**–**RL52**) |
